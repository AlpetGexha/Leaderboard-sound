...
function consolidateResolveAnnouncements(announcements, event) {
  if (announcements.length < 2) return announcements;

  // A plain SOLVED callout adds no excitement next to a special event. Named
  // tiers (for example DOUBLE KILL) remain part of the combined highlight.
  const highlights = announcements.filter(item => !(item.kind === 'tier' && item.title === 'SOLVED'));
  const items = highlights.length ? highlights : announcements;
  const audioSource = items.find(item => item.kind === 'first_blood' || item.kind === 'first_blood_boss_defeated') ||
    items.find(item => item.kind === 'tier' && item.count >= 2) ||
    items.find(item => item.kind === 'urgent_boss_defeated') ||
    items.find(item => item.kind === 'team_combo') ||
    items[0];

  return [{
    ...announcementMeta(event, ':resolve_highlight'),
    kind: 'resolve_highlight', agent: event.agent, service: event.service,
    title: items.map(item => item.title).join(' — '),
    line: items.map(item => item.line).join(' '),
    sourceKinds: items.map(item => item.kind),
    sampleKind: audioSource.kind,
    sampleCount: audioSource.count
  }];
}
...
function applyEvent(state, event, options = {}) {
  ...
  if (event.type === 'ticket.created') {
    ...
    if (announce.urgentBossAnnouncements && (event.priority || 'medium') === 'urgent') {
      announcements.push({
        ...announcementMeta(event, ':urgent_boss_arrival'), kind: 'urgent_boss_arrival',
        agent: event.agent, service: event.service, title: 'URGENT BOSS INCOMING',
        line: formatLine(
          announce.boss.arrival || 'Urgent Boss incoming! {name} opened a critical {service} ticket!',
          event,
          { title: 'URGENT BOSS INCOMING', sound: 'URGENT BOSS' }
        )
      });
    } else {
      announcements.push({
  ...
  if (event.type === 'ticket.resolved') {
    ...
    const firstBlood = !state.firstBlood;
    const urgentBossDefeat = announce.urgentBossAnnouncements && matched?.priority === 'urgent';
    const firstBloodBossDefeat = firstBlood && urgentBossDefeat;
    if (firstBlood) {
      state.firstBlood = { agent: event.agent, service: event.service, ts: event.ts };
      if (firstBloodBossDefeat) {
        announcements.push({
          ...announcementMeta(event, ':first_blood_boss_defeated'),
          kind: 'first_blood_boss_defeated', agent: event.agent, service: matched.service,
          title: 'FIRST BLOOD — BOSS DEFEATED',
          line: formatLine(
            announce.boss.firstBloodDefeated || 'First Blood! {name} has defeated the SLA Apocalypse on {service}!',
            event,
            { title: 'FIRST BLOOD — BOSS DEFEATED', sound: 'FIRST BLOOD' }
          )
        });
      } else {
        announcements.push({
    ...
    if (urgentBossDefeat && !firstBloodBossDefeat) {
      announcements.push({
    ...
    return {
      accepted: true, announcements: consolidateResolveAnnouncements(announcements, event),
      effects: matched
