...
function applyEvent(state, event, options = {}) {
  ...
  if (event.type === 'ticket.created') {
    ...
    state.activeTickets.set(event.ticketId, {
      ticketId: event.ticketId, service: event.service, agent: event.agent, ts: event.ts
    });
    if (!state.firstBlood) {
...
module.exports = { TIERS, dayKey, createDay, applyEvent, publicState, calculateAwards };
