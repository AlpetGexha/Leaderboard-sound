...
function parseWebhook(body, agents) {
  ...
  const requestedPriority = String(body.priority ?? body.status ?? 'medium').trim().toLowerCase();
  const priority = PRIORITIES.has(requestedPriority) ? requestedPriority : 'medium';
  return {
    ok: true,
    event: {
...
module.exports = { PRIORITIES, parseWebhook };
