...
function createArenaServer({
  ...
  const resolvedSoundDir = path.resolve(soundDir);
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && url.pathname === '/api/events') {
      ...
      const { accepted, announcements } = applyEvent(state, parsed.event, applyOptions);
      if (accepted) {
    ...
    if ((req.method === 'POST' || req.method === 'GET') && url.pathname === '/api/tts') {
      if (!fishTts) return sendJson(res, 503, { error: 'tts not configured' });
      let body;
      if (req.method === 'GET') {
        body = {
      ...
      if (!text || text.length > 240) return sendJson(res, 400, { error: 'bad text' });
      try {
    ...
    if (req.method === 'GET' && url.pathname.startsWith('/sound/')) {
      return serveFileFrom(res, resolvedSoundDir, url.pathname.replace(/^\/sound\/?/, ''));
    }

    if (req.method === 'GET') return serveFileFrom(res, resolvedPublicDir, url.pathname, 'index.html');
