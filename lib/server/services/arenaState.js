'use strict';
const { createDay, applyEvent, publicState, dayKey, calculateAwards, DEFAULT_TEAM_COMBOS } = require('../../engine');

function feature(config, name) {
  return config.features?.[name] !== false;
}

// Every flag the frontend/README document must be listed here so it still
// defaults to enabled when config.json omits it entirely. Any additional
// key present in config.json's features object is forwarded automatically,
// so a new flag no longer requires a second manual edit in this file.
const KNOWN_FEATURES = [
  'inboxInvasion', 'comebackAnnouncements', 'endOfDayAwards',
  'urgentBossAnnouncements', 'teamCombos', 'livingBoard'
];
...
function createArenaState({ config, store, now = Date.now, logger = console, onDayRoll = () => {}, onCeremony = () => {} }) {
  const applyOptions = {
    ...
    urgentBossAnnouncements: feature(config, 'urgentBossAnnouncements'),
    boss: config.announcements?.boss,
    teamCombos: feature(config, 'teamCombos'),
    teamComboWindowSeconds: config.featureSettings?.teamComboWindowSeconds ?? 30,
    teamComboMilestones: config.featureSettings?.teamComboMilestones || DEFAULT_TEAM_COMBOS
  };
  const featureNames = new Set([...KNOWN_FEATURES, ...Object.keys(config.features || {})]);
  const features = Object.fromEntries([...featureNames].map(name => [name, feature(config, name)]));
