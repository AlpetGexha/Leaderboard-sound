...
Urgent Boss Announcements add fullscreen boss-arrival and defeat callouts for `urgent` tickets. A new urgent ticket fires one merged `URGENT BOSS INCOMING` announcement rather than separate New Ticket and boss-spawn callouts. Resolve-side callouts are also consolidated into one fullscreen announcement: First Blood, named tiers, urgent-boss defeat, team-combo, and comeback achievements become one richer headline and voice line. A generic `SOLVED` callout is omitted whenever a special achievement occurs. Customize boss lines, including `arrival` and `firstBloodDefeated`, under `announcements.boss`. Team Combos count solves by the whole team inside `teamComboWindowSeconds`; each entry in `teamComboMilestones` configures the triggering count, title, and line. Disabling either feature does not affect solves or leaderboard scoring.
